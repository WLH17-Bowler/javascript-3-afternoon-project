...
let totalPopulation = populations.reduce((initial, aggregate) => {
  return initial + aggregate
})
...
let myStrongest = monstersInYourPocket.filter((element, index, monstersInYourPocket) => {
  return (element.CP > 200 ? element.CP : null)
})
...
let orderTotals = orders.map((element, index, orders) => {
...
*/
let bobsTotal = purchases.filter((key, val, purchases) => {
  let {Bob} = purchases.owner
  return (key === bob ? key.val : null)
})